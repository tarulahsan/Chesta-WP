...
function milliondollartheme_render_posts_display_block( $attributes ) { /* ... posts display render ... */ return '<p>Posts Display Block placeholder.</p>'; }
...
if ( ! function_exists( 'milliondollartheme_ai_dashboard_menu' ) ) {
    function milliondollartheme_ai_dashboard_menu() {
        add_menu_page( __( 'AI Dashboard', 'milliondollartheme' ), __( 'AI Dashboard', 'milliondollartheme' ), 'manage_options', 'milliondollartheme-ai-dashboard', 'milliondollartheme_ai_dashboard_page', 'dashicons-brain', 25 );
    }
...
if ( ! function_exists( 'milliondollartheme_ai_dashboard_page' ) ) {
    function milliondollartheme_ai_dashboard_page() {
        ...
        // Handle Meta Description Generator Form Submission
        ...
        if ( isset( $_POST['milliondollartheme_meta_desc_nonce'] ) && wp_verify_nonce( sanitize_text_field( wp_unslash($_POST['milliondollartheme_meta_desc_nonce']) ), 'milliondollartheme_generate_meta_desc' ) ) {
            if ( !empty($current_openai_api_key) && isset( $_POST['mdt_source_text'] ) && function_exists('milliondollartheme_call_openai_api') ) {
                ...
                if ( !empty($source_text) ) {
                    ...
                    $ai_args = array( 'max_tokens' => 70, 'temperature' => 0.5 );
                    $api_response = milliondollartheme_call_openai_api( $prompt, $ai_args );
                ...
                $meta_generator_error = esc_html__( 'OpenAI API Key is not set. Please configure it first.', 'milliondollartheme' );
            }
        }
        ?>
        <div class="wrap mdt-ai-dashboard">
            <h1><?php esc_html_e( 'MillionDollarTheme AI Dashboard', 'milliondollartheme' ); ?></h1>
            <p><?php esc_html_e( 'Welcome to the AI Dashboard. Configure your API keys and access powerful AI tools for content creation and SEO.', 'milliondollartheme' ); ?></p>
                ...
                <div id="dashboard-widgets" class="metabox-holder">
                    ...
                    <div id="postbox-container-1" class="postbox-container">
                        <div class="meta-box-sortables">
                            <div class="postbox">
                                <h2 class="hndle"><span><?php esc_html_e( 'API Key Settings', 'milliondollartheme' ); ?></span></h2>
                                <div class="inside">
                                    <form method="POST" action="">
                                        <?php wp_nonce_field( 'milliondollartheme_save_openai_api_key', 'milliondollartheme_openai_api_key_nonce' ); ?>
                                        <table class="form-table">
                                            <tr valign="top">
                                                <th scope="row"><label for="milliondollartheme_openai_api_key"><?php esc_html_e( 'OpenAI API Key', 'milliondollartheme' ); ?></label></th>
                                                <td>
                                                    <input type="text" id="milliondollartheme_openai_api_key" name="milliondollartheme_openai_api_key" value="<?php echo esc_attr( $current_openai_api_key ); ?>" class="regular-text" />
                                                    <p class="description"><?php esc_html_e( 'Enter your OpenAI API key. Get it from your OpenAI account dashboard.', 'milliondollartheme' ); ?></p>
                                                </td>
                                            </tr>
                                        </table>
                                    ...
                                    <?php if ( !empty($current_openai_api_key) ) : ?>
                                        <p style="color: green;"><?php esc_html_e( 'OpenAI API Key is set.', 'milliondollartheme' ); ?></p>
                                        <p><small><?php printf( esc_html__( 'Current key starts with: %s', 'milliondollartheme' ), esc_html( substr( $current_openai_api_key, 0, 7 ) . '...' ) ); ?></small></p>
                                    <?php else : ?>
                                        <p style="color: red;"><?php esc_html_e( 'OpenAI API Key is NOT set.', 'milliondollartheme' ); ?></p>
                                    <?php endif; ?>
                                    <hr>
                                    <p><?php esc_html_e( 'Support for other AI services (Gemini, Anthropic, OpenRouter) will be added here.', 'milliondollartheme' ); ?></p>
                                </div>
                            </div>
                        </div>
                    </div>
                    ...
                    <div id="postbox-container-2" class="postbox-container">
                        <div class="meta-box-sortables">
                            <div class="postbox">
                                <h2 class="hndle"><span><?php esc_html_e( 'AI SEO Tools', 'milliondollartheme' ); ?></span></h2>
                                <div class="inside">
                                    <p><?php esc_html_e( 'Tools to help you optimize your content for search engines.', 'milliondollartheme' ); ?></p>
                                    <hr>
                                    <form method="POST" action="" class="mdt-ai-tool-form">
                                        <?php wp_nonce_field( 'milliondollartheme_generate_meta_desc', 'milliondollartheme_meta_desc_nonce' ); ?>
                                        <h4><?php esc_html_e( 'Meta Description Generator', 'milliondollartheme' ); ?></h4>
                                        <p><label for="mdt_source_text"><?php esc_html_e( 'Paste your source text (e.g., blog post introduction or summary):', 'milliondollartheme' ); ?></label></p>
                                        <textarea id="mdt_source_text" name="mdt_source_text" rows="6" class="widefat"><?php echo isset($_POST['mdt_source_text']) ? esc_textarea(wp_unslash($_POST['mdt_source_text'])) : ''; ?></textarea>
                                        <p class="description"><?php esc_html_e( 'The AI will generate a concise, SEO-friendly meta description (around 155 characters) based on this text.', 'milliondollartheme' ); ?></p>
                                        <?php submit_button( __( 'Generate Meta Description', 'milliondollartheme' ), 'secondary', 'generate_meta_description' ); ?>
                                    </form>
                                    ...
                                    <?php if ( $generated_meta_description ) : ?>
                                        <div class="notice notice-success inline">
                                            <h4><?php esc_html_e( 'Suggested Meta Description:', 'milliondollartheme' ); ?></h4>
                                            <p id="generated-meta-desc"><?php echo $generated_meta_description; ?></p>
                                            <button type="button" class="button button-small" onclick="navigator.clipboard.writeText(document.getElementById('generated-meta-desc').innerText)"><?php esc_html_e( 'Copy to Clipboard', 'milliondollartheme' ); ?></button>
                                        </div>
                                    <?php endif; ?>
                                    <hr>
                                    <p><?php esc_html_e( 'More SEO tools (title generator, keyword suggestions) will be added here.', 'milliondollartheme' ); ?></p>
                                </div>
                            </div>
                        </div>
                    </div>
                    ...
                    <div id="postbox-container-3" class="postbox-container">
                        <div class="meta-box-sortables">
                            <div class="postbox">
                                <h2 class="hndle"><span><?php esc_html_e( 'AI Content Tools', 'milliondollartheme' ); ?></span></h2>
                                <div class="inside">
                                    <p><?php esc_html_e( 'Generate content ideas, outlines, or even draft paragraphs.', 'milliondollartheme' ); ?></p>
                                    <p><a href="#" class="button button-secondary"><?php esc_html_e( 'Explore Content Tools (TODO)', 'milliondollartheme' ); ?></a></p>
                                </div>
                            </div>
                        </div>
                    </div>
            ...
            <style>
                /* Basic styling for the dashboard page for now */
                .mdt-ai-dashboard #dashboard-widgets.metabox-holder { display: grid; grid-template-columns: repeat(auto-fit, minmax(350px, 1fr)); gap: 20px; }
                .mdt-ai-dashboard .postbox-container { width: 100%; }
                .mdt-ai-tool-form textarea { margin-bottom: 5px; }
                .mdt-ai-tool-form .description { margin-top: 0; margin-bottom: 15px; }
                .mdt-ai-dashboard .notice { margin-top: 15px; margin-bottom: 15px; padding: 10px; }
                .mdt-ai-dashboard .notice h4 { margin-top: 0; margin-bottom: 5px; }
                .mdt-ai-dashboard .notice button { margin-top: 5px; }
            </style>
...
if ( ! function_exists( 'milliondollartheme_call_openai_api' ) ) {
    function milliondollartheme_call_openai_api( $prompt, $args = array() ) {
        ...
        $request_body = array( 'model' => $merged_args['model'], 'prompt' => $prompt, 'max_tokens' => intval( $merged_args['max_tokens'] ), 'temperature' => floatval( $merged_args['temperature'] ), );
        $api_url = 'https://api.openai.com/v1/completions';
        $response = wp_remote_post( $api_url, array( 'method'  => 'POST', 'headers' => array( 'Authorization' => 'Bearer ' . $api_key, 'Content-Type'  => 'application/json', ), 'body' => json_encode( $request_body ), 'timeout' => 30, ) );
        if ( is_wp_error( $response ) ) { return $response; }
        $response_code = wp_remote_retrieve_response_code( $response );
        $response_body = wp_remote_retrieve_body( $response );
        $decoded_body = json_decode( $response_body, true );
        if ( $response_code !== 200 ) { $error_message = isset( $decoded_body['error']['message'] ) ? $decoded_body['error']['message'] : $response_body; return new WP_Error( 'api_http_error', sprintf(__( 'OpenAI API Error (%1$s): %2$s', 'milliondollartheme' ), $response_code, $error_message) ); }
        if (isset($decoded_body['choices'][0]['text'])) { return trim($decoded_body['choices'][0]['text']);
        } elseif (isset($decoded_body['choices'][0]['message']['content'])) { return trim($decoded_body['choices'][0]['message']['content']);
        } else { return new WP_Error( 'api_unexpected_response', __( 'Unexpected response format from OpenAI API.', 'milliondollartheme' ) ); }
    }
}
