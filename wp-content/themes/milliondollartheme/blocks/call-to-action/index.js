...
registerBlockType(name, {
    ...
    edit: ({ attributes, setAttributes, isSelected }) => {
        ...
        const blockProps = useBlockProps({
            ...
            style: {
                ...
                backgroundColor: (ctaStyle === 'default' && !backgroundImageUrl && !gradient) ? backgroundColor : undefined,
        ...
        const overlayStyles = {
            ...
            opacity: backgroundImageUrl ? (overlayOpacity / 100) : 0, // Only apply opacity if there is an image
        ...
        return (
            <>
                <InspectorControls>
                    ...
                    <PanelBody title={__('Layout & Style', 'milliondollartheme')}>
                        ...
                        <SelectControl
                            ...
                            value={ctaStyle}
                            options={[ {label: 'Default', value: 'default'}, {label: 'Glassy', value: 'glassy'}, {label: 'Gradient Background', value: 'gradient-bg'} ]}
                            onChange={(val) => setAttributes({ ctaStyle: val })}
                        />
                    </PanelBody>
                    <PanelBody title={__('Background', 'milliondollartheme')}>
                        {ctaStyle === 'default' && <TextControl /*TODO: ColorPalette*/ label={__('Background Color', 'milliondollartheme')} value={backgroundColor || ''} onChange={val => setAttributes({backgroundColor: val})} />}
                        {ctaStyle === 'gradient-bg' && <TextareaControl /*TODO: GradientPicker*/ label={__('Background Gradient', 'milliondollartheme')} value={gradient} onChange={val => setAttributes({gradient: val})} />}
                        <MediaUploadCheck>
                            <MediaUpload
                                onSelect={(media) => setAttributes({ backgroundImageUrl: media.url, backgroundImageId: media.id })}
                                allowedTypes={['image']} value={attributes.backgroundImageId}
                        ...
                        {backgroundImageUrl && <>
                            ...
                            <TextControl /*TODO: ColorPalette*/ label={__('Overlay Color', 'milliondollartheme')} value={overlayColor} onChange={val => setAttributes({overlayColor: val})} />
                    ...
                    <PanelBody title={__('Text Colors', 'milliondollartheme')}>
                        <TextControl /*TODO: ColorPalette*/ label={__('Heading Color', 'milliondollartheme')} value={headingColor || ''} onChange={val => setAttributes({headingColor: val})} />
                        <TextControl /*TODO: ColorPalette*/ label={__('Text Color', 'milliondollartheme')} value={textColor || ''} onChange={val => setAttributes({textColor: val})} />
                    </PanelBody>
                    <PanelBody title={__('Button 1', 'milliondollartheme')}>
                        ...
                        {showButton1 && <>
                            ...
                            <SelectControl label={__('Button 1 Style', 'milliondollartheme')} value={button1Style} options={[{label: 'Primary', value: 'primary'}, {label: 'Outline', value: 'outline'}, {label: 'Glassy', value: 'glassy'}]} onChange={val => setAttributes({button1Style: val})} />
                            <ToggleControl label={__('Open in new tab', 'milliondollartheme')} checked={!!button1NewTab} onChange={() => setAttributes({button1NewTab: !button1NewTab})} />
                        </>}
                    </PanelBody>
                    <PanelBody title={__('Button 2 (Optional)', 'milliondollartheme')}>
                        ...
                        {showButton2 && <>
                            ...
                            <SelectControl label={__('Button 2 Style', 'milliondollartheme')} value={button2Style} options={[{label: 'Primary', value: 'primary'}, {label: 'Outline', value: 'outline'}, {label: 'Glassy', value: 'glassy'}]} onChange={val => setAttributes({button2Style: val})} />
                            <ToggleControl label={__('Open in new tab', 'milliondollartheme')} checked={!!button2NewTab} onChange={() => setAttributes({button2NewTab: !button2NewTab})} />
                        </>}
                    </PanelBody>
                ...
                <div {...blockProps}>
                    {backgroundImageUrl && <div className="cta-overlay" style={overlayStyles}></div>}
                    <div className="cta-content-wrapper" style={contentWrapperStyles}>
                        <div className="cta-text-content">
                            <RichText tagName={HeadingTag} className="cta-heading" value={heading} onChange={(val) => setAttributes({ heading: val })} placeholder={__('Enter Heading...','milliondollartheme')} style={headingStyles} />
                            <RichText tagName="p" className="cta-text" value={text} onChange={(val) => setAttributes({ text: val })} placeholder={__('Enter text...','milliondollartheme')} style={textStyles} />
                        </div>
                        {(showButton1 || showButton2) && (
                            <div className="cta-buttons">
                                {showButton1 && (
                                    <a href={button1Url} className={`cta-button is-style-${button1Style}`}> {/* Preview, not a real link */}
                                        {button1Text}
                                        {isSelected && <URLInputButton url={button1Url} onChange={(url) => setAttributes({ button1Url: url })} />}
                                    </a>
                                )}
                                {showButton2 && (
                                    <a href={button2Url} className={`cta-button is-style-${button2Style}`}> {/* Preview */}
                                        {button2Text}
    ...
    save: ({ attributes }) => {
        ...
        const overlayStyles = {
            backgroundColor: overlayColor,
            opacity: backgroundImageUrl ? (overlayOpacity / 100) : 0,
            position: 'absolute', top: 0, left: 0, right: 0, bottom: 0, zIndex: 1,
            borderRadius: borderRadius
        };
        const contentWrapperStyles = { position: 'relative', zIndex: 2 };
